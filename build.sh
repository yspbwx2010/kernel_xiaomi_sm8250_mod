...
for path in "${SEARCH_PATHS[@]}"; do
  echo "正在搜索: $path"
  found=$(find "$path" -maxdepth 5 -type d -name 'llvm21.0.0-binutils2.43.1_amd64-full-toolchain-20250207' -print -quit 2>/dev/null)
...
if [[ -z "$TOOLCHAIN_PATH" ]]; then
  echo "❌ 错误：未找到 llvm21.0.0-binutils2.43.1_amd64-full-toolchain-20250207 目录"
  echo "请尝试以下方法："
  echo "1. 确认已下载并解压工具链"
...
if [ $KSU_ENABLE -eq 1 ]; then
    echo "KSU is enabled"
    curl -LSs "https://raw.githubusercontent.com/rifsxd/KernelSU-Next/refs/heads/next/kernel/setup.sh" | bash -s
else
...
sed -i "s/${local_version_str}//" arch/arm64/configs/${TARGET_DEVICE}_defconfig
